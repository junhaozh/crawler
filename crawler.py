...
# code modified from original indeed code
def get_job_links_from_page(url):
    ...
    for item in soup.find_all("a", href=True):
        if 'link-identifier=' in str(item) and 'absolute' in str(item):
            link = item['href']
            job_page_links.append(head+link)
